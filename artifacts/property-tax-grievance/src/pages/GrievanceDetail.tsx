...
import { generateAppealArgument, getStateStrategy, calculateApprovalLikelihood } from "@/utils/generate-argument";
...
export function GrievanceDetail() {
  ...
  const appealArgument = generateAppealArgument(
    {
      address: grievance.propertyAddress,
      assessedValue: grievance.currentAssessment,
      estimatedMarketValue: grievance.estimatedMarketValue,
      county: grievance.county,
      state: grievanceState,
    },
    comparables.map((c) => ({ salePrice: c.salePrice }))
  );
  const strategy = getStateStrategy(grievanceState);
  const approvalLikelihood = calculateApprovalLikelihood(
    grievance.estimatedMarketValue,
    grievance.currentAssessment,
    comparables.length
  );
  ...
  return (
    <AppLayout>
        ...
        <div className={`rounded-2xl border p-5 ${confidence.bgColor}`}>
          <div className="flex items-center justify-between mb-1">
            <h3 className="font-serif font-bold text-base flex items-center gap-2">
              <Sparkles className="w-4 h-4" /> Approval Likelihood
            </h3>
            <span className={`text-2xl font-extrabold font-serif ${approvalLikelihood.color}`}>{approvalLikelihood.score}%</span>
          </div>
          <p className={`text-xs font-semibold mb-3 ${approvalLikelihood.color}`}>
            Confidence Level: {approvalLikelihood.label}
          </p>
          <div className="w-full bg-white/60 rounded-full h-2 mb-3 overflow-hidden">
            <div
              className={`h-2 rounded-full transition-all duration-700 ${approvalLikelihood.label === "High" ? "bg-emerald-500" : approvalLikelihood.label === "Moderate" ? "bg-amber-500" : "bg-slate-400"}`}
              style={{ width: `${approvalLikelihood.score}%` }}
            />
          </div>
          ...
          {approvalLikelihood.label === "High" && (
            <p className="text-xs font-semibold text-red-600 mt-2 flex items-center gap-1">
              <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
              High-confidence cases are best filed before the deadline
            </p>
          )}
      ...
      {/* ── Appeal Argument Card ── */}
      <div className="mb-6 bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-border bg-blue-50/60 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-800 text-xs font-bold px-3 py-1 rounded-full mb-1.5">
              <Sparkles className="w-3 h-3" /> Appeal Strategy: {strategy.title}
            </div>
            <h2 className="font-serif font-bold text-base">Your Appeal Argument</h2>
            <p className="text-xs text-muted-foreground mt-0.5">{strategy.description}</p>
          </div>
          {comparables.length > 0 && (
            <span className="text-xs font-medium text-blue-700 bg-blue-100 px-2.5 py-1 rounded-full whitespace-nowrap">
              Based on {comparables.length} comparable{comparables.length !== 1 ? "s" : ""}
            </span>
          )}
        </div>
        <div className="p-5">
          <p className="text-sm text-foreground leading-relaxed">{appealArgument}</p>
          <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />
            Professionally generated based on {grievanceState} property tax standards
            {comparables.length === 0 && (
              <span className="text-amber-600 ml-1">· Add comparable sales to strengthen this argument</span>
            )}
          </p>
          {comparables.length === 0 && (
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
              <strong>Strengthen your argument:</strong> Adding 3–6 recent comparable sales from your neighborhood will significantly improve this statement and your approval likelihood.
            </div>
          )}
          {avgCompPrice !== null && (
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-secondary/40 rounded-xl p-3 text-center">
                <p className="text-xs text-muted-foreground mb-1">Avg. Comp Value</p>
                <p className="font-bold text-sm">${avgCompPrice.toLocaleString()}</p>
              </div>
              <div className="bg-secondary/40 rounded-xl p-3 text-center">
                <p className="text-xs text-muted-foreground mb-1">Current Assessment</p>
                <p className="font-bold text-sm text-red-700">${grievance.currentAssessment.toLocaleString()}</p>
              </div>
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-center">
                <p className="text-xs text-emerald-700 mb-1">Assessment Gap</p>
                <p className="font-bold text-sm text-emerald-800">${Math.max(0, grievance.currentAssessment - avgCompPrice).toLocaleString()}</p>
              </div>
            </div>
          )}
        </div>
                ...
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="bg-white border border-border shadow-2xl rounded-2xl p-8 max-w-sm w-full text-center space-y-4 mx-4">
                      ...
                      {/* ── Disclaimers ── */}
                      <div className="text-xs text-gray-500 leading-relaxed text-left border-t border-gray-100 pt-3 space-y-2">
