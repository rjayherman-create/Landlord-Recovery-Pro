...
async function lookupNYSOrps(
  ...
  // Owner name — ORPS stores the FULL name in primary_owner_last_name only.
  // Format is "LASTNAME, FIRSTNAME MIDDLE" for individuals, or just "COMPANY NAME" for entities.
  // primary_owner_first_name is always empty in the dataset.
  const rawOwner = String(best.primary_owner_last_name || "").trim();
  let ownerFull = "";
  if (rawOwner) {
    if (rawOwner.includes(",")) {
      // "GYENIS, CSABA & K KONISZEWSKI" → "Csaba & K Koniszewski Gyenis"
      // Keep display simple: just Title Case the whole thing as written
      ownerFull = toTitleCase(rawOwner);
    } else {
      ownerFull = toTitleCase(rawOwner);
    }
  }
  ...
  // Full market value — many NY municipalities leave this at 0 in ORPS.
  // Only use it when it's a plausible positive number.
  const fmvRaw = parseFloat(best.full_market_value || "0");
  // Sanity check: FMV should be > 0. Accept even if it seems low (old data).
  if (fmvRaw > 0) {
    result.estimatedMarketValue = Math.round(fmvRaw);
    fieldsFound.push("estimatedMarketValue");
  }

  // Lot size from ORPS front × depth (lot frontage × lot depth in feet)
  const front = parseFloat(best.front || "0");
  const depth = parseFloat(best.depth || "0");
  const lotSqft = front > 0 && depth > 0 ? Math.round(front * depth) : 0;
  if (lotSqft > 0) {
    const acres = (lotSqft / 43560).toFixed(3);
    result.lotSize = `${front}×${depth} ft (≈${lotSqft.toLocaleString()} sq ft / ${acres} acres)`;
    fieldsFound.push("lotSize");
  }
