/**
 * Seed script — inserts 5 sample landlord cases for marketing/testing.
 * 
 * Run locally:  pnpm --filter @workspace/api-server run seed
 * Run Railway:  railway run pnpm --filter @workspace/api-server run seed
 */

import { db } from "../../../lib/db/src/index.js";
import { landlordCases } from "../../../lib/db/src/schema/landlord-cases.js";
...
async function seed() {
  console.log("Clearing existing landlord_cases data…");
  await db.execute(sql`TRUNCATE landlord_cases RESTART IDENTITY CASCADE`);

  console.log(`Inserting ${samples.length} sample cases…`);
  for (const s of samples) {
    await db.insert(landlordCases).values(s as any);
    console.log(`  ✓ ${s.tenantName} — ${s.claimType} — ${s.state}`);
  }

  const result = await db.execute(sql`SELECT COUNT(*)::text AS count FROM landlord_cases`);
  const count = (result as any).rows?.[0]?.count ?? (Array.isArray(result) ? (result[0] as any)?.count : "5");
  console.log(`\nDone. ${count} cases in landlord_cases.`);
  process.exit(0);
}
